...
- `Context\RequestJobContext` is the provider-neutral value object for request, command, and queued-job observability. Keep only shared trace, actor, operation, tenant, and hidden idempotency identifiers as first-class fields; owning domains add visible metadata through its generic bag. Use `add()` for the current context and `scope()` for temporary context so Laravel propagates it to queued jobs.
...
- Boolean active/inactive state uses `Filament\Forms\Components\ActiveToggle` in schemas and `Filament\Tables\Columns\ActiveToggleColumn` in tables. Both default to the `active` column, the Bolt on-icon, and the shared `vendra-support::attributes.active` label; pass another name (`ActiveToggle::make('in_stock')`) and chain `->label()` or `->default()` per resource instead of rebuilding `Toggle` / `ToggleColumn` chains.
