...
- `IsDefaultToggle`, `IsDefaultEntry`, and `Filament\Tables\Filters\QueryBuilder\Constraints\IsDefaultConstraint` are the shared `is_default` controls: default name `is_default` and the shared label; the toggle mirrors `IsActiveToggle` but defaults to off. Choosing which record is the default stays in the owning package's action.
